...
```{r cognateCodonProportions, fig.width=12, fig.height=6, fig.path='R_plots_codonBiasHELANANObiasPlotOnly/', dev=c('png', 'pdf')}

# Read the codon counts
utr <- read.csv("5utrCodonCounts.csv", header = FALSE, stringsAsFactors = FALSE)
colnames(utr) <- c("Codon", "Count")
# Read the pos1 file (tab-delimited)
pos1 <- read.table("pos1CodonFreqHELANANO.txt", header = FALSE, sep = "\t", stringsAsFactors = FALSE)
...
# Step 8: Prepare long format for plotting
...
p_highlight <- ggplot(df_long_highlight, aes(x = Codon, y = Frequency, fill = Region)) +
  ...
  labs(
    title = "HELANANO: Codon Frequency in 5'UTR for Expressed Genes vs Nm[+1] in Near-Cognate Codons",
    x = "Codon",
    y = "Frequency",
    fill = "5'UTR"
  ) +
